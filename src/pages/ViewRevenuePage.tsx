...
export default function ViewRevenuePage() {
  ...
  const [document, setDocument] = useState<RevenueDocument | null>(null);
  ...
  const loadDocument = async () => {
    ...
    try {
      ...
      const { data: attachmentsData, error: attachmentsError } = await supabase
        ...
        .eq('revenue_document_id', documentId)
      ...
      if (docData.source_type === 'invoice' && docData.source_invoice_id) {
  ...
  const handleDelete = () => {
    ...
    if (document.source_type === 'invoice') {
      alert('Les revenus issus de factures ne peuvent pas être supprimés directement.');
      return;
    }

    setConfirmModal({
      isOpen: true,
      message: 'Confirmer la suppression de ce revenu ? Cette action est irréversible.',
      onConfirm: async () => {
        ...
        try {
          ...
          navigate(`/app/company/${companyId}/revenues`);
        } catch (error) {
  ...
  return (
    <>
      <div style={{ padding: '32px 20px', maxWidth: '1000px', margin: '0 auto' }}>
        <BackButton to={`/app/company/${companyId}/revenues`} />
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ margin: '0 0 8px 0', fontSize: '14px', color: '#1e40af', fontWeight: '600' }}>
                  Revenu issu d'une facture
                </p>
                <p style={{ margin: 0, fontSize: '13px', color: '#1e3a8a', lineHeight: '1.6' }}>
                  Ce revenu a été généré automatiquement depuis une facture payée et ne peut pas être modifié directement.
                </p>
              </div>
            </div>
          </div>
