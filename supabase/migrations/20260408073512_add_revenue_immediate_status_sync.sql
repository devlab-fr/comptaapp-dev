/*
  # Ajouter synchronisation des statuts pour les revenus immédiats

  1. Objectif
    - En mode "immediate", marquer automatiquement le document comme payé et validé
    - Le revenu est encaissé au moment de la création, pas de créance à gérer

  2. Logique ajoutée
    - APRÈS la création de l'écriture comptable
    - HORS du bloc BEGIN/EXCEPTION (pour éviter les rollbacks)
    - Si v_is_immediate AND v_entry_id IS NOT NULL
      → Mettre à jour payment_status = 'paid' ET accounting_status = 'validated'

  3. Placement critique
    - Ce bloc doit être placé APRÈS le UPDATE linked_accounting_entry_id
    - EN DEHORS du bloc BEGIN/EXCEPTION
    - Utiliser IS DISTINCT FROM pour éviter les updates inutiles
*/
