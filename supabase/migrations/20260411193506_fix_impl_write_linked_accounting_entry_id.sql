/*
  # Patch — auto_create_revenue_accounting_entry_impl : écriture du lien linked_accounting_entry_id

  ## Problème
  La fonction _impl crée correctement l'écriture comptable (accounting_entries via source_document_id),
  mais n'écrit jamais revenue_documents.linked_accounting_entry_id.
  Conséquence : l'UI affiche "Aucune écriture comptable liée" même quand l'écriture existe.

  ## Correction
  Ajouter un UPDATE revenue_documents SET linked_accounting_entry_id = v_entry_id
  après la vérification d'équilibre débit/crédit, avant le bloc de synchronisation statuts immediate.
  Position exacte : après IF total_debit != total_credit, avant IF v_is_immediate.
*/
