/*
  # PATCH TEMPORAIRE — Tracer erreur réelle trigger paiement

  1. Contexte
    - Document test différé : payment_status=paid, accounting_status=draft
    - payment_entry_id reste NULL
    - Aucune écriture 401→512 créée
    - Bloc EXCEPTION avale l'erreur silencieusement

  2. Modification
    - Remplacer EXCEPTION WHEN OTHERS pour faire remonter l'erreur exacte
    - PATCH TEMPORAIRE DE DIAGNOSTIC
    - Ne modifie RIEN d'autre dans la fonction

  3. Test à faire après
    - Marquer une dépense différée comme payée
    - Récupérer SQLSTATE et SQLERRM exacts
    - Identifier la cause réelle du blocage
*/
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    PERFORM set_config('app.batch_accounting_insert', 'false', true);
    RAISE EXCEPTION 'ERREUR TRIGGER PAIEMENT | SQLSTATE=% | SQLERRM=%', SQLSTATE, SQLERRM;
  END;
